export default function Header({
  ...
  this.$target = document.createElement("div"); //target 변수에 div 요소 할당
  this.$target.className = "header"; //클래스 이름은 header
  ...
  this.template = () => {
    ...
    if (!currentPage.includes("/city/"))
      //if문을 쓰는 이유: 상세페이지에서 정렬기능과 검색기능을 없애기 위해
  ...
  this.setState = (newState) => {
    this.state = newState; //현재 상태를 newstate로 업데이트 시켜줌
    this.render(); //랜더
  };

  this.render(); // 랜더함수 다시 호출
}
